\hypertarget{noise-calculation}{%
\section{Noise Calculation}\label{noise-calculation}}

Notebook to make calculations in
\url{https://www.renesas.com/ja/document/apn/r13an0010-noise-calculations-op-amp-circuits}
...
We can calculate the 1/f noise voltage and current corner frequencies
using: \[f_{ce}=f_{xe} ((\frac{e_{nf(fe)}}{e_{nw}})^2-1)\]
\[f_{ci}=f_{xi} ((\frac{i_{nf(fi)}}{i_{nw}})^2-1)\]

The white noise equivalent bandwidth

For thermal noise \[E_R = \sqrt(4kTR(nf_H - f_L))\]
